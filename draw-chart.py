...
def fetcher_factory(url, auth):
    ...
    def get_issue(key):
        ...
        resource = Resource(url + ('/rest/api/latest/issue/%s' % key), pool_instance=None, filters=[auth])
    ...
    def walk(issue_key, graph):
        ...
        if fields.has_key('subtasks'):
            for subtask in issue['fields']['subtasks']:
                node = '"%s"->"%s"[color=blue][penwidth=2.0]' % (get_key(issue), get_key(subtask))
                graph.append(node)
                children.append(subtask['key'])
        if fields.has_key('issuelinks') :
            for link in issue['fields']['issuelinks']:
                if link.has_key('outwardIssue'):
                    node = '"%s"->"%s"[arrowhead=dot]' % (get_key(issue), get_key(link['outwardIssue']))
                    graph.append(node)
